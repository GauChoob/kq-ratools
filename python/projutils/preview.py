...
valid_coll_ids = [x for row in valid_coll_ids for x in row]
...
DUMMY_ASSETS_ASM = "assets/scenes/scenes/_python_preview_assets.asm"
...
def _preview(scene_label: str, scene_address: utils.BankAddress, sym: utils.SymFile):
    """Previews a single scene"""
    print(scene_label)

    # Read Scene file
    scenery = scene.Scene.init_from_rom(sym, rom, scene_address)
    if scene_label == 'Scene_dResh_Puzzle2_Mask':
        # Since this one map changes a lot, render the mask maps instead (hardcoded)
        scenery.metamap.bankaddress = utils.BankAddress(0x2C, 0x7AA0)
        scenery.collmap.bankaddress = utils.BankAddress(0x2D, 0x6F25)
    ...
    # TEMP
    coll_id_set = set(collmap.map.contents.map)
    for id in coll_id_set:
        if id not in valid_coll_ids:
            print(f'{id:02X}')

    # Setup Collisionmap
    collmap_height = collmap.approx_size//collmap.width
    if collmap.width != metamap.width or collmap_height != metamap_height:
        print(f'Mismatch between Metamap {metamap.width}x{metamap_height} and Collmap {collmap.width}x{collmap_height}. Replacing with null collmap')
        collmap.map.contents.map = bytearray([0x11 for x in range(metamap.width) for y in range(metamap_height)])
        collmap_height = metamap_height
        collmap.width = metamap.width
...
def _load_scene_assets() -> None:
    ...
    assetlist.registerFromAsm(DUMMY_ASSETS_ASM)
...
def preview_one(scene_label: str, scene_address: utils.BankAddress, sym: utils.SymFile) -> None:
    """Generate a preview of a specific scene."""
    _setup()
    _preview(scene_label, scene_address, sym)
